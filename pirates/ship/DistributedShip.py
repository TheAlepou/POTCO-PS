...
class DistributedShip(DistributedMovingObject, DistributedCharterableObject, ZoneLOD.ZoneLOD, Teamable):
    ...
    def generate(self):
        if self.hasCachedData('DistributedShip'):
            ...
            for sailProjector in list(self.sailProjectors.values()):
    ...
    def _doSailAlpha(self):
        for d in list(self.sails.values()):
            for (sail, distSail) in list(d.values()):
    ...
    def getShipInfo(self):
        return [self.doId, self.getName(), self.shipClass, [ [dMast.getMastType(), dMast.getPosIndex(), dMast.getSailConfig()] for mast, dMast in list(self.masts.values()) if dMast ]]

    def getLocalShipInfo(self):
        return [self.doId, self.getName(), self.shipClass, [ [mast.dna.getMastType(), mast.dna.getPosIndex(), mast.dna.getSailConfig()] for mast, dMast in list(self.masts.values()) ]]
